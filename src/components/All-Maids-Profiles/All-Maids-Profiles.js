...
const AllMaidsProfiles = ({ searchTerm , filterSearchTerm, selectedCountries, selectedReligions, selectedLanguages, selectedExperiences }) => {
  ...
  useEffect(() => {
    const fetchMaidProfiles = async () => {
      try {
        const response = await axiosInstense.get(`api/v1/maids?category=${category ? category : ""}`, {
          params: {
    ...
    fetchMaidProfiles();
  }, [searchTerm, category, filterSearchTerm, selectedCountries, selectedReligions, selectedLanguages, selectedExperiences]);
