...
const MaidDetailMainSection = (props) => {
  ...
  return (
    ...
    <path d="M19 12L5 12" stroke="#262F32" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        ...
                        <span className="text-base md:text-2xl font-bold">{maidDetails.name}</span>
                      </div>
                      <div className="flex items-center gap-2 sm:gap-4">
                      <a className="decoration-transparent" href={`${process.env.REACT_APP_API_URL}cv/pdf/${maidID}`}>
                      <div className="bg-[#FFFBFA] cursor-pointer p-3 flex items-center gap-1 text-base rounded-lg">
                      <span>
                      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
  <path d="M4 17V19C4 19.5304 4.21071 20.0391 4.58579 20.4142C4.96086 20.7893 5.46957 21 6 21H18C18.5304 21 19.0391 20.7893 19.4142 20.4142C19.7893 20.0391 20 19.5304 20 19V17M7 11L12 16M12 16L17 11M12 16V4" stroke="#434146" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
</svg>
</span><span className="text-xs sm:text-base text-[#262F32]">Download CV</span></div>
                      </a>
                      
                      <div className="bg-[#107243] text-[#FFFBFA] cursor-not-allowed p-3 flex items-center gap-1 text-base rounded-lg">
                      <span className="text-xs sm:text-base">Plan interview</span></div>
                      </div>
                    </div>
                        <div className="w-full border border-solid bg-white rounded-lg p-4 md:p-6">
                                        ...
                                        <div className="maidDetails overflow-y-auto max-w-full">
                                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 overflow-x-auto gap-y-4 md:gap-y-8 lg:gap-y-12 gap-x-8 mt-4">
                                                <div className="nationality">
                                                    ...
                                                    <div className="text-sm font-semibold">{maidDetails.name}</div>
                                                </div>
                                                <div className="nationality">
                                                ...
                                                <div className="Salary">
                                                    ...
                                                    <div className="text-sm font-semibold">{maidDetails.salery} OMR</div>
                                                </div>
                            ...
                            <div className="my-8">
                                <div className="maidImages overflow-x-auto">
                                    ...
                                    {maidDetails.maidImg2 && <div className="w-[18rem] md:w-[15rem] flex-shrink-0 lg:flex-shrink flex items-center justify-center rounded-lg bg-[#F2F2F2]">
                                            <img alt="maid 2" className="w-full rounded-lg h-[13rem] md:h-[21rem] md:w-full p-2 object-cover object-top" src={`${backApiUrl}${maidDetails.maidImg2}`}/>
                                        </div>
                                    }
                                    {maidDetails.maidImg3 && <div className="w-[18rem] md:w-[15rem] flex-shrink-0 lg:flex-shrink flex items-center justify-center rounded-lg bg-[#F2F2F2]">
                                            <img alt="maid 3" className="w-full  h-[13rem] md:h-[21rem] md:w-full p-2 object-cover object-top" src={`${backApiUrl}${maidDetails.maidImg3}`}/>
                                        </div>
                                    }
                                    {maidDetails.maidImg4 && <div className="w-[18rem] md:w-[15rem] flex-shrink-0 lg:flex-shrink flex items-center justify-center rounded-lg bg-[#F2F2F2]">
                                            <img alt="maid 4" className="w-full  h-[13rem] md:h-[21rem] md:w-full p-2 object-cover object-top" src={`${backApiUrl}${maidDetails.maidImg4}`}/>
                                        </div>
                                    }
                                    
                                    {maidDetails.videoLink.includes("youtube.com") || maidDetails.videoLink.includes("youtu.be") ? (
              <YouTube videoId={extractYouTubeVideoId(maidDetails.videoLink)} opts={opts} className="w-full border border-solid shadow-md rounded-2xl h-[13rem] md:h-[21rem] md:w-full p-2" />
            ) : (
              <video
                                                controls
